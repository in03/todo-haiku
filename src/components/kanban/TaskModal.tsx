...
interface TaskModalProps {
  ...
  defaultStatus?: TaskStatus; // For new tasks when adding from a specific column
}

export const TaskModal = component$<TaskModalProps>(({ open, onClose, task, defaultStatus = 'open' }) => {
  const isEditing = useSignal(!!task);
  const content = useSignal(task?.content || '');
  const title = useSignal(task?.title || '');
  const status = useSignal<TaskStatus>(task?.status || defaultStatus);

  const validation = useStore({
    isValid: !!task, // Assume existing tasks are valid
  ...
  useVisibleTask$(({ track }) => {
    ...
    if (open) {
      ...
      // Update form values if editing an existing task
      if (task) {
        title.value = task.title || '';
        content.value = task.content || '';
        status.value = task.status || defaultStatus;

        // Validate the haiku
        const result = validateHaiku(task.content || '');
  ...
  // Submit the task (create or update)
  const submitTask = $(() => {
    if (!validation.isValid || !title.value.trim()) {
      return;
    }

    if (isEditing.value && task) {
  ...
  return (
    ...
    >
      <div class="space-y-4">
        ...
        {/* Only show status selector when editing or when no defaultStatus is provided */}
