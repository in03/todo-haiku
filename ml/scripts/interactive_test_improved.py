"""
Interactive testing script for the improved CMU-based syllable counter model.
"""
import torch
import json
import time
...
class SyllablePredictor:
    def __init__(self, model_path="models/syllable_counter_cmu_improved.pt", vocab_path="models/char_vocab_cmu_improved.json"):
        self.console = Console(width=80)  # Set fixed width
        self.metrics_history: list[PredictionMetrics] = []
        self.model, self.vocab, self.bias_correction = self.load_model_and_vocab(model_path, vocab_path)
    ...
    def load_model_and_vocab(self, model_path, vocab_path):
        ...
        self.console.print(f"[green]Bias correction: {bias_correction}[/green]")
        
        return model, vocab, bias_correction
    ...
    def predict_word(self, word: str) -> PredictionMetrics:
        ...
        with torch.no_grad():
            ...
            _, raw_pred = torch.max(outputs, 1)
            ...
            if self.bias_correction != 0:
                predicted = max(1, raw_pred - self.bias_correction)
                print(f"After bias correction (-{self.bias_correction}): {predicted} syllables")
            else:
                predicted = raw_pred
    ...
    def display_metrics(self, metrics: PredictionMetrics):
        ...
        # Show raw prediction
        table.add_row("Raw Prediction", str(metrics.raw_prediction))
