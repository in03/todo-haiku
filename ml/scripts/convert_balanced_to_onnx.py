"""
Convert the balanced syllable counter model to ONNX format.
"""
import argparse
import os
import json
...
def convert_to_onnx(
    ...
    # Load vocabulary
    vocab_path = os.path.join(os.path.dirname(model_path), 'char_vocab_balanced.json')
    ...
    torch.onnx.export(
        ...
        os.path.join(output_dir, 'syllable_counter_balanced.onnx'),
        export_params=True,
        opset_version=12,
    ...
    print(f"Model exported to {os.path.join(output_dir, 'syllable_counter_balanced.onnx')}")
    
    # Save vocabulary as JSON
    with open(os.path.join(output_dir, 'char_vocab_balanced.json'), 'w') as f:
        json.dump(char_to_idx, f)
    
    print(f"Vocabulary saved to {os.path.join(output_dir, 'char_vocab_balanced.json')}")
    ...
    with open(os.path.join(output_dir, 'model_metadata_balanced.json'), 'w') as f:
        json.dump(metadata, f)
    
    print(f"Metadata saved to {os.path.join(output_dir, 'model_metadata_balanced.json')}")
