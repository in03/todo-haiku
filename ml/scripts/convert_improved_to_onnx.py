"""
Convert the improved CMU-based syllable counter model to ONNX format.
"""
import argparse
import os
import json
...
def convert_to_onnx(
    ...
    # Get bias correction
    bias_correction = checkpoint.get('bias_correction', 0)
    print(f"Bias correction: {bias_correction}")
    
    # Load vocabulary
    vocab_path = os.path.join(os.path.dirname(model_path), 'char_vocab_cmu_improved.json')
    ...
    # Create a custom model class that includes bias correction
    class SyllableCounterWithBiasCorrection(torch.nn.Module):
        def __init__(self, model, bias_correction):
            super().__init__()
            self.model = model
            self.bias_correction = bias_correction
        
        def forward(self, char_ids, features):
            outputs = self.model(char_ids, features)
            # We don't apply bias correction here because ONNX doesn't support argmax
            # The bias correction will be applied in the JavaScript code
            return outputs
    
    # Create the model with bias correction
    model_with_bias = SyllableCounterWithBiasCorrection(model, bias_correction)
    
    # Export model to ONNX
    torch.onnx.export(
        model_with_bias,
        (dummy_char_ids, dummy_features),
        os.path.join(output_dir, 'syllable_counter_cmu_improved.onnx'),
        export_params=True,
        opset_version=12,
    ...
    print(f"Model exported to {os.path.join(output_dir, 'syllable_counter_cmu_improved.onnx')}")
    
    # Save vocabulary as JSON
    with open(os.path.join(output_dir, 'char_vocab_cmu_improved.json'), 'w') as f:
        json.dump(char_to_idx, f)
    
    print(f"Vocabulary saved to {os.path.join(output_dir, 'char_vocab_cmu_improved.json')}")
    ...
    with open(os.path.join(output_dir, 'model_metadata_cmu_improved.json'), 'w') as f:
        json.dump(metadata, f)
    
    print(f"Metadata saved to {os.path.join(output_dir, 'model_metadata_cmu_improved.json')}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_path",
        type=str,
        default="models/syllable_counter_cmu_improved.pt",
